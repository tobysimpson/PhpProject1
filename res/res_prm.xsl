        ...
        <table class="table1" onload="fn_get('prm.php?mth=plot&amp;res_id={row/@res_id}&amp;prm_id={row/@prm_id}',disp);">
            ...
            <xsl:for-each select="row">
                <tr>
                    ...
                    <td>
                        <input type="range" value="{format-number(@du, '###,##0.000')}"  min="-1" max="1" step="0.05" onchange="window.location.assign('prm.php?mth=ups1&amp;res_id={@res_id}&amp;prm_id={@prm_id}&amp;p={@p}&amp;du='+this.value)"/>
                    </td>
                    <td>
                        <xsl:value-of select="format-number(@du, '0.000')"/>
                    </td>
                    <td>
                        <input type="range" value="{format-number(@u, '###,##0.000')}"  min="-1" max="1" step="0.05" onchange="window.location.assign('prm.php?mth=ups2&amp;res_id={@res_id}&amp;prm_id={@prm_id}&amp;p={@p}&amp;u1='+this.value)"/>
                    </td>
                    <td>
