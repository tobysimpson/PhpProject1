...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:variable name="vv" select="root/row/@m8" />
        ...
        <svg width="{$w}" height="{$h}" xmlns="http://www.w3.org/2000/svg" >
            ...
            <g id="plot" transform="translate({$wo},{$ho})"> 
                ...
                <xsl:call-template name="line1">
                    <xsl:with-param name="att1" select="'m8'" />
                </xsl:call-template>
                <xsl:call-template name="line1">
                    <xsl:with-param name="att1" select="'m7'" />
                </xsl:call-template>
                <xsl:call-template name="line1">
                    <xsl:with-param name="att1" select="'m6'" />
                </xsl:call-template>
                <xsl:call-template name="line1">
                    <xsl:with-param name="att1" select="'m5'" />
                </xsl:call-template>
                <xsl:call-template name="line1">
                    <xsl:with-param name="att1" select="'m4'" />
                </xsl:call-template>
                <xsl:call-template name="line1">
                    <xsl:with-param name="att1" select="'m3'" />
                </xsl:call-template>
                <xsl:call-template name="line1">
                    <xsl:with-param name="att1" select="'m2'" />
                </xsl:call-template>
                <xsl:call-template name="line1">
                    <xsl:with-param name="att1" select="'m1'" />
                </xsl:call-template>
