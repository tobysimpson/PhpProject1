...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="root">
        <svg width="{$w}" height="{$h}" xmlns="http://www.w3.org/2000/svg" >
            ...
            <text x="0" y="20" alignment-baseline="hanging" style="font-weight: bold;">temperature</text>
            
            <g id="plot" transform="translate(00,40)"> 
                ...
                <xsl:variable name="v_min">
                    <xsl:for-each select="row">
                        <xsl:sort select="@tmp_v1" data-type="number" order="ascending"/>
                        <xsl:if test="position() = 1">
                            <xsl:value-of select="@tmp_v1"/>
                        </xsl:if>
                    </xsl:for-each>
                </xsl:variable>
                <xsl:variable name="v_max">
                    <xsl:for-each select="row">
                        <xsl:sort select="@tmp_v1" data-type="number" order="descending"/>
                        <xsl:if test="position() = 1">
                            <xsl:value-of select="@tmp_v1"/>
                        </xsl:if>
                    </xsl:for-each>
...
<!--                <g id="key">
                ...
                <g id="labels">
                    ...
                    <text x="{$pw + 10}" y="{format-number($ph * (1 - (row[last()]/@tmp_v1 - $v_min) div $v_rng),'0.0')}" alignment-baseline="middle" style="font-weight: bold;">
                        <xsl:value-of select="format-number(row[last()]/@tmp_v1,'0.000')"/>
                    </text>
                </g>
                ...
                <g id="line1">
                    <xsl:variable name="line1">
                        <xsl:for-each select="row">
                            ...
                            <xsl:variable name="y" select="format-number($ph * (1 - (@tmp_v1 - $v_min) div $v_rng),'0.0')"/>
                            <xsl:choose>
