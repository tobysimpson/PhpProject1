...
//        $a4 = array($p[3] * $q[3], $p[3] * (1 - $q[3]), (1 - $p[3]) * $q[3], (1 - $p[3]) * (1 - $q[3]));
        //transposed
        $a1 = array("a11"=>$p[0] * $q[0]            ,"a12"=> $p[1] * $q[2]              ,"a13"=> $p[2] * $q[1]              ,"a14"=> $p[3] * $q[3]);
        $a2 = array("a21"=>$p[0] * (1 - $q[0])      ,"a22"=> $p[1] * (1 - $q[2])        ,"a23"=> $p[2] * (1 - $q[1])        ,"a24"=> $p[3] * (1 - $q[3]));
        $a3 = array("a31"=>(1 - $p[0]) * $q[0]      ,"a32"=> (1 - $p[1]) * $q[2]        ,"a33"=> (1 - $p[2]) * $q[1]        ,"a34"=> (1 - $p[3]) * $q[3]);
        $a4 = array("a41"=>(1 - $p[0]) * (1 - $q[0]),"a42"=> (1 - $p[1]) * (1 - $q[2])  ,"a43"=> (1 - $p[2]) * (1 - $q[1])  ,"a44"=> (1 - $p[3]) * (1 - $q[3]));
